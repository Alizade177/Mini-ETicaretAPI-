    ...
    public class AuthService : IAuthService
    {
        ...
        async Task<Token> CreateUserExternalAsync(AppUser user,string email,string name,UserLoginInfo info,int accessTokenLifeTime)
        {
            ...
            if (result)
            {
                ...
                await _userService.UpdateRefreshTokenAsync(token.RefreshToken,user,token.Expration,15);
                return token;
            }
        ...
        public async Task<Token> RefreshLoginAsync(string refreshToken)
        {
           AppUser? user =  _userManager.Users.FirstOrDefault(u => u.RefreshToken == refreshToken);
            if(user != null && user?.RefreshTokenEndDate > DateTime.UtcNow)
            {
               ...
               await _userService.UpdateRefreshTokenAsync(token.RefreshToken,user,token.Expration,300);
               return token;
            }
            else
             throw new NotFoundUserException();
